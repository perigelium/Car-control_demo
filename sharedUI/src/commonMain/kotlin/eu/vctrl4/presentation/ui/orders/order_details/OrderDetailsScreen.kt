...
@Composable
fun OrderDetailsScreen(order: OrderDetails, titleTexts:List<IdNameValueName>, titleTextsTranspTop:List<IdNameValueName>,
                       titleTextsTranspOptionTypes:List<IdNameValueName>, titleTextsTranspBottom:List<IdNameValueName>,
                       onBackPressed:() -> Unit, onPerformAction:(String) -> Unit, navigate:(String) -> Unit)
{
    ...
    val remStrSubtitle: MutableState<String> = remember { mutableStateOf(BOTTOM_APP_BAR_NAV_ITEMS.get(remSelItemIndex.intValue).title) }
    val stFabImageVector: MutableState<ImageVector> = remember { mutableStateOf(Icons.AutoMirrored.Default.ArrowForward) }

    Surface(modifier = Modifier.fillMaxSize(), shape = RectangleShape, Colors.white) {
        Column(
            modifier = Modifier.fillMaxSize().background(Colors.cl_white), horizontalAlignment = Alignment.CenterHorizontally
        ) {
            val titleText = if (order.Number != null) "No. " + order.Number else Res.string.order.asState
            ...
            order.State?.let {
                Row(
                    ...
                    Text(
                        text = ORDER_STATES_MAP.get(order.State)?:"", color = Colors.white, fontSize = 14.sp, maxLines = 1
                    )
                }
                Box(
            ...
            Scaffold(modifier = Modifier.background(color = Colors.white).fillMaxSize(), floatingActionButton = { // .nestedScroll(scrollBehavior.nestedScrollConnection)
                ...
                Box() {
                    FloatingActionButton(
	                    onClick = {
                            ...
                            if (curStepNumber == stepsCount - 1 || curStepNumber == 0) forwardBackward = !forwardBackward
                            ...
                            stFabImageVector.value = if (forwardBackward) Icons.AutoMirrored.Default.ArrowForward else Icons.AutoMirrored.Default.ArrowBack
                        },
	                    shape = CircleShape,
	                    modifier = Modifier.align(Alignment.Center).size(56.dp).offset(y = 40.dp),
	                    containerColor = Colors.cl_00549F,
	                    contentColor = Color.White,
	                    elevation = FloatingActionButtonDefaults.bottomAppBarFabElevation(),
                    ) {
                        Icon(
                            imageVector = stFabImageVector.value, contentDescription = null, modifier = Modifier.size(24.dp)
                        )
                    }
                }
                ...
                BottomAppBarOrderDetails(
                    items = BOTTOM_APP_BAR_NAV_ITEMS, remSelItemIndex = remSelItemIndex, onNavigationItemClick = { route ->
                        navigate(route)
                        curStepNumber = BOTTOM_APP_BAR_NAV_ITEMS.indexOf(
                            (BOTTOM_APP_BAR_NAV_ITEMS.firstOrNull { it.name == route }) ?: 0
                        )
                        remStrSubtitle.value = BOTTOM_APP_BAR_NAV_ITEMS.get(curStepNumber).title
                    })
            },
